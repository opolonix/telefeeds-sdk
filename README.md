...
Каталог доступен только пока подключён живой интерфейс. SDK отвечает на ping
clientshub автоматически; отсутствие pong в течение 30 секунд закрывает канал.
При отключении последнего канала интерфейса каталог инвалидируется. Для этого
протокола необходим SDK 0.3.1; старый протокол без heartbeat не поддерживается.
